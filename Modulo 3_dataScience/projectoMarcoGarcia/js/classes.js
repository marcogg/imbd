...
// WOrking from here to line 167
// Get grades from students array
const getGrades = (arrStudents) => {
    let arrGrades = []
    for (let i = 0; i < arrStudents.length; i++) {
        if (arrStudents[i].score) {
            arrGrades.push(arrStudents[i])
        }
    }
    // If only one student is registred, print that data in table
    ...
    // If there is more than one student in the array
    // Quick Sort
    let pivot = arrStudents[0]
    let left = []
    let right = []

    for (let i = 1; i < arrStudents.length; i++) {
        if (arr[i] < pivot) {
            left.push(arrStudents[i])
        } else {
            right.push(arrStudents[i])
        }
    }
...
// Event listener for sort increasing
let getSortIncreasing = document.querySelector('#increasing')
getSortIncreasing.addEventListener('"click', sortIncreasing(students))
